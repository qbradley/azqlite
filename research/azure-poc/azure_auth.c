/*
 * azure_auth.c — HMAC-SHA256 Shared Key authentication for Azure Storage REST API
 *
 * Implements the StringToSign construction and HMAC-SHA256 signing
 * per https://learn.microsoft.com/en-us/rest/api/storageservices/authorize-with-shared-key
 *
 * Dependencies: OpenSSL (for HMAC-SHA256), standard C library
 */
...
/* ----------------------------------------------------------------
 * String comparison for sorting x-ms- headers (case-insensitive)
 * ---------------------------------------------------------------- */
...
/* ----------------------------------------------------------------
 * Build StringToSign and compute Authorization header
 *
 ...
 *   Content-Encoding\n
 *   Content-Language\n
 *   Content-Length\n         (omit if zero for GET/HEAD; include if non-zero)
 *   Content-MD5\n
 *   Content-Type\n
 *   Date\n                   (empty when x-ms-date is used)
 *   If-Modified-Since\n
 *   If-Match\n
 *   If-None-Match\n
 *   If-Unmodified-Since\n
 *   Range\n
 *   CanonicalizedHeaders\n   (sorted x-ms-* headers, one per line)
    ...
    size_t auth_header_size)
{
     ...
     * We set Date to empty because we always use x-ms-date instead
    ...
    if (x_ms_headers) {
        /* Count and copy headers for sorting */
        int count = 0;
        ...
        for (int i = 0; i < count; i++) {
            /* Each header: "x-ms-name:value" — name must be lowercase */
            ...
            for (const char *c = h; c < colon; c++) {
                if (p < end - 1) *p++ = (char)tolower((unsigned char)*c);
            }
            /* Write :value\n */
     ...
     * If query params exist, append them sorted: \nparam:value
     */
    p += snprintf(p, (size_t)(end - p), "/%s%s", client->account, path);

    if (query && *query) {
        /*
         * Parse query string and sort parameters.
         * Azure requires: \ncomp:page\ntimeout:30  (sorted by param name)
         */
        char qbuf[1024];
        strncpy(qbuf, query, sizeof(qbuf) - 1);
        qbuf[sizeof(qbuf) - 1] = '\0';

        /* Simple parse: split by & into param=value pairs */
        char *pairs[32];
        int npairs = 0;
        char *saveptr;
